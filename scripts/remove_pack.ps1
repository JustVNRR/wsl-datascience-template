...
Write-Host ""
Write-Host "Packs installed in '$DistroName':" -ForegroundColor Cyan
for ($Index = 0; $Index -lt $Installed.Count; $Index++) {
    Write-Host ("  {0,2}.  {1}" -f ($Index + 1), $Installed[$Index])
}
Write-Host "   0.  Cancel"
...
while (-not $PackName) {
    ...
    if ([int]::TryParse($Answer.Trim(), [ref]$Number)) {
        ...
        if ($Number -ge 1 -and $Number -le $Installed.Count) {
            $PackName = $Installed[$Number - 1]
        }
    }
    if (-not $PackName) { Write-Host "  '$Answer' is not one of the numbers above." -ForegroundColor Yellow }
}
